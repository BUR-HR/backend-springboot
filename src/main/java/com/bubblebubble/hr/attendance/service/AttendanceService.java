...
public class AttendanceService {
    ...
    private LocalDateTime startOfWeek = now.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY))
            .with(LocalTime.MIN);

    // 현재 날짜의 마지막 시간 계산 (현재 주의 금요일)
    private LocalDateTime endOfWeek = now.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY))
            .with(LocalTime.MAX);
    ...
    public AttendanceDTO updateEndDateTime(AttendanceDTO attendanceDTO) throws AttendanceInfoNotFoundException {
        ...
        Attendance attendance = attendanceRepository
                .findTopByEmpNoAndStartDateTimeBetween(attendanceDTO.getEmpNo(), startDateTime, endDateTime)
